...
if not os.path.exists(ccfg.mirror_flat_filename):
    answer = raw_input('Mirror flat file %s not found. Create all-zero flat file? [Y/n] '%ccfg.mirror_flat_filename)
...
if not os.path.exists(ccfg.poke_filename):
    answer = raw_input('Poke file %s not found. Create all-one poke file? [Y/n] '%ccfg.poke_filename)
    if answer.lower() in ['y','']:
...
if (not poke.shape[0]==poke_n_rows) or (not poke.shape[1]==poke_n_cols):
    answer = raw_input('Poke shape (%d,%d) disagrees with 2x number of lenslets (%d) and number of actuators (%d). Create all-one poke file? [Y/n] '%(poke.shape[0],poke.shape[1],n_lenslets*2,n_actuators))
    if answer.lower() in ['y','']:
        poke = np.zeros((n_lenslets*2,n_actuators))
        noise = np.random.randn(n_lenslets*2,n_actuators)*1e-6
        poke = poke+noise
